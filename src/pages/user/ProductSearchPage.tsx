...
const ProductSearchPage = () => {
    ...
    useEffect(() => {
        const getProducts = async () => {
            ...
            setIsEmpty(!data.length ? true : false);
            setProducts(data);
        };
        getProducts();
    }, [keyword]);
    ...
    return (
        <>
            <section className="container max-w-6xl mx-auto px-3 pt-8 mb-5 text-center">
                ...
                <h1 className="text-[#D9A953] font-semibold text-3xl mt-1">{isEmpty ? `Không tìm thấy kết quả nào cho "${keyword}"` : `Kết quả tìm kiếm "${keyword}"`}</h1>
            </section>
