                ...
                {categories?.map((item, index) => (
                    <tr key={index}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <Link to={`/chuyen-muc/${item.slug}`} className="hover:underline">{item.name}</Link>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <Link to={`/admin/category-news/${item.slug}/edit`} className="h-8 inline-flex items-center px-3 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Edit</Link>
                            <button
