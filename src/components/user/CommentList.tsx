...
const CommentList = ({ productId, reRender: productDetailRerender, slug, page }: CommentListProps) => {
    ...
    useEffect(() => {
        const getComments = async () => {
            ...
            const commentJoinRating = dataComment.map((cmt: any) => {
                const rating = dataRating.find((item: any) => item.userId === cmt.userId._id);
    ...
    const handleRemoveCmt = async (cmtId: string) => {
        Swal.fire({
    ...
    return (
        <div>
                    ...
                    <li className="flex py-4" key={index}>
                        ...
                        <div className="ml-2">
                            ...
                            <div>
                                ...
                                <span className="text-sm text-gray-500"> ({formatDateNews(item.createdAt)})</span>
                            </div>
                            <p className="text-gray-500">{item.content}</p>
                            <ul className="text-gray-500 flex text-sm mt-1">
                                {((item.userId === user._id) || user.role) && (
                                    <li
            ...
            <Pagination page={page || 1} totalPage={totalPage} url={`san-pham/${slug}`} />
        </div>
    )
}
