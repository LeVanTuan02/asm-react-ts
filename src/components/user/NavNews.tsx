...
const NavNews = ({ slug }: NavNewsProps) => {
    ...
    return (
        <section className="container max-w-6xl px-3 mx-auto flex mt-8 justify-center">
            <Link
                to="/tin-tuc"
                className={`text-center px-4 group flex flex-col items-center cate-news-item ${!slug && "active"}`}
            >
            ...
            {cateNews?.map((item, index) => (
                <Link
                    ...
                    key={index}
                    className={`text-center px-4 group flex flex-col items-center cate-news-item ${item.slug === slug && "active"}`}
                >
                    <div className="cate-news-icon w-[75px] h-[75px] text-3xl rounded-full flex items-center justify-center bg-[#EEE8DF] transition duration-300 group-hover:bg-[#D9A953] group-hover:text-white cursor-pointer">
                    ...
                    <p className="cate-news-name font-semibold mt-1 group-hover:text-[#D9A953] transition duration-300">{item.name}</p>
                </Link>
            ))}
        </section>
    )
