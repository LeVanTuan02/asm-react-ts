                ...
                <ul className="grid grid-cols-1 divide-y mt-2">
                    {productTop10?.map((item, index) => (
                        <li className="py-3 flex" key={index}>
                            <Link to={`/san-pham/${item.slug}`} className="block bg-[#f7f7f7]">
                                <img
                            ...
                            <div className="ml-3 flex-1">
                                <Link to={`/san-pham/${item.slug}`} className="uppercase transition duration-300 ease-linear hover:text-black block text-[#D9A953] text-sm">{item.name}</Link>
