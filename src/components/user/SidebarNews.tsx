...
const SidebarNews = ({ cateId, newsId }: SidebarNewsProps) => {
    const [cateNews, setCateNews] = useState<CategoryNewsType[]>();
    const [newsLatest, setNewsLatest] = useState<NewsType[]>();
                    ...
                    {cateNews?.map((item, index) => (
                        <li key={index}>
                            <Link
                                ...
                                className={`${cateId === item._id && "text-black font-semibold"} block py-1 leading-7 text-gray-500 transition duration-200 hover:text-black`}
                            >{item.name}</Link>
                        </li>
                    ))}
                </ul>
                    ...
                    {newsLatest?.map((item, index) => (
                        <li key={index}>
                            <Link
                                ...
                                className={`${item._id === newsId && "text-black font-semibold"} limit-line-2 block py-1 leading-7 text-gray-500 transition duration-200 hover:text-black`}
                            >{item.title}</Link>
                        </li>
                    ))}
                </ul>
